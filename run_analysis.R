...
trainSubject<-read.table(file.path(Directory, "./UCI HAR Dataset/train/subject_train.txt"), col.names= "SubjectId")
...
testSubject<-read.table(file.path(Directory, "./UCI HAR Dataset/test/subject_test.txt"), col.names = "SubjectId")
...
measurements<-mergedDataSet[,c("SubjectId","activity",grep("mean|std", names(mergedDataSet), value = TRUE))]
